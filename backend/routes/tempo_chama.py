...
def create_tempo():
    ...
    if data.get('horas') is None or data.get('minutos') is None or data.get('segundos') is None:
    ...
    return jsonify({
        'message': 'Tempo de chama registrado com sucesso',
...
def get_tempo(tempo_id):
    ...
    response = supabase.table("tempo_chama").select("*").eq("id", tempo_id).eq("user_id", user_id).execute()
...
def delete_tempo(tempo_id):
    ...
    existing = supabase.table("tempo_chama").select("id").eq("id", tempo_id).eq("user_id", user_id).execute()
