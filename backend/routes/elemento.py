...
def create_elemento():
    ...
    existing = supabase.table("elemento").select("id").eq("nome", data['nome']).eq("user_id", user_id).execute()
    if existing.data:
...
def get_elemento(elemento_id):
    ...
    response = supabase.table("elemento").select("*").eq("id", elemento_id).eq("user_id", user_id).execute()
...
def update_elemento(elemento_id):
    ...
    existing = supabase.table("elemento").select("id").eq("id", elemento_id).eq("user_id", user_id).execute()
    if not existing.data:
        return jsonify({'message': 'Elemento não encontrado'}), 404
    
    if data.get('nome'):
        duplicate = supabase.table("elemento").select("id").eq("nome", data['nome']).eq("user_id", user_id).neq("id", elemento_id).execute()
        if duplicate.data:
    ...
    response = supabase.table("elemento").update(update_data).eq("id", elemento_id).execute()
